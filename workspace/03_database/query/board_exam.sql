...
INSERT INTO post (member_id, title, content, created_at, view_count) VALUES
    (1, '세 번째 게시글', '오늘도 자바 공부를 열심히 하고 있습니다.', '2026-06-16 13:00:00', 15),
...
INSERT INTO reply (post_id, member_id, content, created_at) VALUES
    (1, 2, '좋은 글이네요.', '2025-06-16 13:30:00'),
...
SELECT p.id, 
	CASE 
		WHEN p.created_at >= DATE_SUB(NOW(), INTERVAL 12 HOUR) THEN CONCAT('NEW ',title)
	...
	p.created_at
FROM post p
INNER JOIN member ON p.member_id = member.id
LEFT JOIN reply r ON p.id = r.post_id 
GROUP BY p.id, p.title, member.name, p.created_at
ORDER BY p.created_at DESC;

-- 작성한지 24시간 이내의 게시글 제목 앞에 (New) 추가
SELECT id, created_at, CONCAT('NEW ',title) AS NEW_TITLE
FROM post
WHERE created_at >= DATE_SUB(NOW(), INTERVAL 12 HOUR);

-- 댓글이 3개 이상 달린 게시글 제목 앞에 (Best) 추가
SELECT p.id, CONCAT('BEST ',p.title), COUNT(r.id) AS reply_count
FROM post p
LEFT JOIN reply r ON p.id = r.post_id
...
SELECT p.title, member.name, member.email, r.name, r.content
FROM post p
